...
function Blog(props) {
    ...
    const [blogType, setBlogType] = useState("date")
    const sortedBlogData = [...data].sort((a, b) => {
        if (blogType === "popular") {
            return a.popular - b.popular;
        }
        return 0;
    });
    const handleTogglePort = (type) => {
        setBlogType(type);
    }
    ...
    return (
        <div id="portfolio" className={`w-full rounded-[15px] bg-[#1C1E20FF] px-4 py-6 lg:px-10 lg:py-12 space-y-4`}>
            ...
            <div className={`pt-2 lg:pt-6 flex lg:space-x-4 lg:space-y-0 space-y-4 lg:flex-row flex-col w-fit justify-start lg:items-center`}>
                <div onClick={() => handleTogglePort("date")} className={`cursor-pointer border border-dashed rounded-xl px-3 py-1  tracking-wider font-mono ${blogType === "date" ? "text-white/90 border-white/90" : "text-white/40 border-white/40"} hover:text-white/80`}>Date</div>
                <div onClick={() => handleTogglePort("popular")} className={`cursor-pointer border border-dashed rounded-xl  px-3 py-1  tracking-wider font-mono ${blogType === "popular" ? "text-white/90 border-white/90" : "text-white/40 border-white/40"} hover:text-white/80`}>Popular</div>
            </div>
